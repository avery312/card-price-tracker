    ...
    try:
        ...
        if edited_rows:
            ...
            for filtered_index, changes in edited_rows.items():
                ...
                # original_date 可能是 date 对象或 None（因为我们已经清洗过）
                if original_date and isinstance(original_date, date):
...
if df.empty:
    ...
    # 填充 NaT 为 None，避免 data_editor 报错
    display_df['date'] = date_series.apply(lambda x: x if pd.notna(x) else None)
