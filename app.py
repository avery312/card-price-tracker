...
def update_data_and_save(edited_df):
    ...
    try:
        ...
        # 确保编辑后的 price 和 quantity 也是正确的数值类型
...
if df.empty:
    ...
    # 【核心修正 2：将日期转换为 YYYY-MM-DD 字符串，以增强与 st.data_editor 的兼容性】
    # 之前尝试使用 date 对象可能不稳定，此处改为最稳定的字符串格式
    ...
    column_config_dict = {
        ...
        # 确保 price 和 quantity 使用 NumberColumn
