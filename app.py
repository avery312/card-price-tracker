...
with st.sidebar:
    ...
    with st.form(key=f"manual_entry_form_{suffix}"):
        card_number_in = st.text_input("1. 卡牌编号 (查询/录入)", value=card_number_in_potential, key=f"card_number_in_manual_{suffix}")
...
if df.empty:
    ...
    # 核心修复: 先转换为 copy, 格式化日期为字符串, 然后 drop date_dt
